...
{
    player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, optionNpc, std::move(text), false, 0, "", sender, action);
}
...
{
    player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, optionNpc, std::move(text), coded, popupMoney, std::move(popupText), sender, action);
}
